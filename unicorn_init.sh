...
# more information.
APP_ROOT=/home/prod_user/nuage_app
...
#UNICORN="bin/unicorn"
UNICORN="cd $APP_ROOT;bundle exec unicorn"
# Execute the unicorn executable as a daemon, with the appropriate configuration
# and in the appropriate environment.
[ -f /home/prod_user/nuage_app/config/.db_secrets ] && . /home/prod_user/nuage_app/config/.db_secrets
...
case $action in
  ...
  start)
    ...
    su - $USER --preserve-environment -c "$CMD"
    ;;
  ...
  restart)
    sig QUIT && echo "Restarting $DESC" && sleep 2 \
      && su - $USER --preserve-environment -c "$CMD" && exit 0
